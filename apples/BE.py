...
class BE(Algorithm):
        ...
        for node in filter(lambda x: x.valid, subtree.traverse_postorder()):
            if node.is_leaf():
                ...
                for child in filter(lambda x: x.valid, node.children):
                    ...
                    node.Sd2_D += (
                        child.S1_D * child.edge_length * child.edge_length
                        + child.Sd2_D
                        + 2 * child.edge_length * child.Sd_D
                    )
                    node.SD += child.SD
    ...
    def all_R_values(self):
        ...
        for node in filter(lambda x: x.valid, subtree.traverse_preorder()):
            ...
            for sibling in filter(lambda x: x.valid and x != node, node.parent.children):
                ...
                node.Rd2_D += (
                    sibling.S1_D * sibling.edge_length * sibling.edge_length
                    + sibling.Sd2_D
                    + 2 * sibling.edge_length * sibling.Sd_D
                )
                node.RD += sibling.SD
            ...
            if node.parent != subtree.root and node.parent.valid:
                ...
                node.Rd2_D += (
                    node.parent.R1_D * node.parent.edge_length * node.parent.edge_length
                    + node.parent.Rd2_D
                    + 2 * node.parent.edge_length * node.parent.Rd_D
                )
    ...
    def error_per_edge(node):
        assert node.valid
        A = node.RD + node.SD
        B = 2 * (node.x_1 + node.x_2) * node.Rd_D + 2 * (node.edge_length + node.x_1 - node.x_2) * node.Sd_D
        C = (node.x_1 + node.x_2) ** 2 * node.R1_D + (node.edge_length + node.x_1 - node.x_2) ** 2 * node.S1_D
        D = -2 * (node.x_1 + node.x_2) * node.R - 2 * (node.edge_length + node.x_1 - node.x_2) * node.S
